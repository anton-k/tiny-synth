...
class St():
    def __init__(self):
        ...
        self.recent = readPickleConfig(inits.recent, self.cfg, Recent())
        self.favs = readPickleConfig(inits.favs, self.cfg, Favorites())
        self.fontSize = readPickleConfig(inits.fontSize, self.cfg, inits.defaultFontSize)
    ...
    def close(self, recent, favs):
        self.cfg.Write(inits.fadersDictName, pickle.dumps(self.fadersDict))
        self.setFavs(favs)
...
def getInstruments():
    cats = map(os.path.basename, glob.glob(inits.instrumentsPath + '/*'))
    
    def getCsds(c):
        files = map(lambda x: stripExt(os.path.basename(x)), glob.glob(inits.instrumentsPath + '/' + c + '/*'))
        return (c, files)
...
def pathToCsdFile(path):
    return os.path.join(inits.instrumentsPath, *path) + u'.csd'
